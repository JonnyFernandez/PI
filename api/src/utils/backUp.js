...
//         const resApi = await axios.get(`${spoonacularURL}/recipes/complexSearch?apiKey=${process.env.API_KEY_2}&addRecipeInformation=true&number=100`);
        
         
//         backOut.push(resApi)
        
//        backOut = (await Promise.all(backOut)).map(res=>res.data.results.map(item=>{
//             return({
//                 id: item.id,  
//                 name: item.title,
//                 image: item.image,                 
...
//                 steps: (item.analyzedInstructions[0] && item.analyzedInstructions[0].steps?item.analyzedInstructions[0].steps.map(item=>item.step).join(" \n"):''),
//                 diets: item.diets?.map(element => element), 
...
//------------------------------------------------------------------------------------------------
    const fakeApi=()=>{
        ...
        let arr =[{
            ...
            "image": "https://www.lastingredient.com/wp-content/uploads/2023/03/asparagus-salad-cannellini-beans5.jpg",//
